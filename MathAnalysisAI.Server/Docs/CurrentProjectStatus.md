...
- R26 生产容器化部署（进行中）
  - 当前部署链路已升级到 `.NET 10`，`server` 镜像与运行时均使用 `10.0` 基线。
  ...
  - `ForwardedHeaders__KnownNetworks` 继续沿用兼容键名，代码侧已写入 `KnownIPNetworks`。
...
- 2026-06-12 阶段一稳定化补充完成：
  - 认证模式已真正收口到后端：`DevelopmentUsername` 仅允许 Development 环境使用；`Disabled / LocalPassword / Oidc` 会返回清晰错误，不再伪装成“用户名错误”
  - `CurrentUserService` 的 Development fallback 已绑定 `Auth:Mode=DevelopmentUsername`，不会再在其他认证模式下误生效
  - 前端已统一 OCR / LLM / timeout / config / auth 相关失败文案，用户可以看到更明确的失败原因
  - `auth.js` 与 `analysis.js` 的前端开发 fallback 已收紧，不再因为本地 host 判断而绕过服务端认证模式
  - 部署说明已同步到 `.NET 10`、`KnownIPNetworks` / `ForwardedHeaders__KnownNetworks` 兼容写法，以及当前 `server.env` 必需项
- 现阶段后续建议优先级：
  1. 错误码前端文案统一
  2. 首页 / 题目页知识点标签风格统一
  3. 进一步补充数学分析题型策略库
