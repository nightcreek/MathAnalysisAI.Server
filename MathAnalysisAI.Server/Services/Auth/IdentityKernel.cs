...
public sealed class IdentityKernel : IIdentityKernel
{
    ...
    public IdentityKernel(
        ...
        IPermissionPersistenceService permissionPersistenceService,
    ...
    public async Task<IdentityAuthenticationResult> AuthenticateAsync(string username, string? password, CancellationToken cancellationToken = default)
    {
        var normalizedUsername = username?.Trim();
        if (string.IsNullOrWhiteSpace(normalizedUsername))
        {
            return IdentityAuthenticationResult.Failure(400, "AUTH_USERNAME_REQUIRED", "Username is required.");
        }

        var user = await _authPersistenceService.FindUserByUsernameAsync(normalizedUsername, cancellationToken);
        if (user == null)
        {
            return IdentityAuthenticationResult.Failure(401, "AUTH_INVALID_CREDENTIALS", "Invalid username or password.");
        }
        ...
        if (_authOptions.IsLocalPasswordMode())
        {
            ...
            {
                return IdentityAuthenticationResult.Failure(400, "AUTH_PASSWORD_REQUIRED", "Password is required.");
            }
            ...
            return IdentityAuthenticationResult.Success(user);
        }
        ...
        return IdentityAuthenticationResult.Failure(503, "AUTH_MODE_UNAVAILABLE", "当前部署未启用该登录入口。");
    }

    public async Task<bool> CanViewRealStudentInfoAsync(int viewerUserId, int targetStudentUserId, int courseId, CancellationToken cancellationToken = default)
    {
        ...
        return viewer.Role == AppUserRole.Student && viewerUserId == targetStudentUserId;
    }

    public async Task<bool> CanViewCourseLeaderboardWithRealNamesAsync(int viewerUserId, int courseId, CancellationToken cancellationToken = default)
    {
        ...
        return viewer.Role == AppUserRole.Teacher;
    }
