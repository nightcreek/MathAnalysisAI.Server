...
(function () {
  function resolveEndpoint(target) {
    if (typeof target === "string") {
      return { url: target, contract: null };
    }

    if (target && typeof target === "object") {
      return {
        url: target.endpoint || target.url || "",
        contract: target
      };
    }

    return { url: "", contract: null };
  }
  ...
  async function requestDetailed(target, init) {
    var resolved = resolveEndpoint(target);
    var requestInit = Object.assign({}, init || {});
    requestInit.headers = buildHeaders(requestInit.headers, null);
    var res = await fetch(resolved.url, requestInit);
    var parsed = await parseResponse(res);
    if (resolved.contract && window.ApiContractValidator && typeof window.ApiContractValidator.validate === "function") {
      window.ApiContractValidator.validate(resolved.contract, parsed.data);
    }
    return parsed;
  }

  async function requestJsonDetailed(method, target, payload) {
    return requestDetailed(target, {
      method: method,
  ...
  async function requestFormDataDetailed(target, formData) {
    return requestDetailed(target, {
      method: "POST",
  ...
  async function get(target) {
    var result = await requestDetailed(target, { method: "GET" });
    return result.data;
  }

  async function getDetailed(target) {
    return requestDetailed(target, { method: "GET" });
  }

  async function post(target, payload) {
    var result = await requestJsonDetailed("POST", target, payload);
    return result.data;
  }

  async function postDetailed(target, payload) {
    return requestJsonDetailed("POST", target, payload);
  }

  async function put(target, payload) {
    var result = await requestJsonDetailed("PUT", target, payload);
    return result.data;
  }

  async function putDetailed(target, payload) {
    return requestJsonDetailed("PUT", target, payload);
  }

  async function del(target) {
    var result = await requestDetailed(target, { method: "DELETE" });
    return result.data;
  }

  async function postFormData(target, formData) {
    var result = await requestFormDataDetailed(target, formData);
    return result.data;
  }

  async function postFormDataDetailed(target, formData) {
    return requestFormDataDetailed(target, formData);
  }

  function fetchWithAuth(target, init) {
    var resolved = resolveEndpoint(target);
    var requestInit = Object.assign({}, init || {});
    requestInit.headers = buildHeaders(requestInit.headers, null);
    return fetch(resolved.url, requestInit);
  }
